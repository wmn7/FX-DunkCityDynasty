...
class Network(RecurrentNetwork, nn.Module):
    ...
    def __init__(self,
        ...
        self.rnn_output = model_config["rnn_output"]
        ...
        #     self.common_layers["embeding_act"] = nn.ReLU()
        #
        #     # 设置 RNN模块
        ...
        #                                        batch_first=True)
        #
        #     self.common_layers["GNN_0"] = nn.ModuleList(
        ...
        #
        #     self.common_layers["GNN_1"] = nn.ModuleList(
        ...
        #                        nn.ReLU(),
        #                        nn.Linear(self.action_policy_size, self.action_dim))
        #         for _ in range(self.player_typenum)
        #     ])
        #
        ...
        #                        nn.ReLU(),
        #                        nn.Linear(self.value_policy_size, 1))
        #         for _ in range(self.player_typenum)
        #     ])
        #
        ...
        if self.common_layers["type_bedding"] is None:
            # 设置5个embedding
            self.type_bedding = nn.ModuleList()
            for _ in range(self.player_typenum):
                self.type_bedding.append(nn.Linear(self.agent_obs_size, self.embedding_size))
            self.embeding_act = nn.ReLU()
            ...
            self.solo_value_nn = nn.ModuleList([
                nn.Sequential(nn.Linear(self.GNN_output_size, self.value_policy_size),
                               nn.ReLU(),
                               nn.Linear(self.value_policy_size, 1))
                for _ in range(self.player_typenum)
            ...
            self.team_value_nn = nn.ModuleList([
                nn.Sequential(nn.Linear(self.GNN_output_size, self.value_policy_size),
                               nn.ReLU(),
                               nn.Linear(self.value_policy_size, 1)) for _ in range(self.player_typenum)
            ])
